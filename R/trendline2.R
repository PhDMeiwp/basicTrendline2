...
#'
#' trendline2(x1,y1,model="line2P",summary=TRUE,eDigit=10)
#' trendline2(x2,y2,model="line3P",summary=FALSE,ePos="topright")
#' trendline2(x3,y3,model="log2P",linecolor="blue")
#' trendline2(x4,y4,model="exp3P",eSize=0.7) #change the font size of equation.
#' trendline2(x5,y5,model="power3P")
#'
#' ## Not run
...
trendline2 <- function(x, y, model="line2P", Pvalue.corrected=TRUE, linecolor="red", lty=1, lwd=1, summary=TRUE, ePos="topleft", eDigit=5, eSize=1, plot=TRUE, ...)
{
  model=model
  if (plot==TRUE){
  plot(x, y,...)
